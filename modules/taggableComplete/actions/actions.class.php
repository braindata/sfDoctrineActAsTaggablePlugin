...
class taggableCompleteActions extends sfActions
{
  ...
  public function executeCompleteTag(sfWebRequest $request)
  {
    ...
    foreach ($results as $tag) {
      $suggestedTags[] = $tag->getName();
    }

    return $this->renderText(json_encode($suggestedTags));
  }
    ...
    return $this->renderText(json_encode($this->getTags($object)));
	}
  ...
  {
      $assigned_tags = $object->getTags();
      $popular_tags = array();
      $popular_tags_count = TagTable::getAllTagNameWithCount(null, array('model' => get_class($object), 'sort_by_popularity' => true, 'limit' => 10));
      foreach ($popular_tags_count as $tag => $count)
      {
          if (!in_array($tag, $assigned_tags))
              $popular_tags[] = $tag;
      }

      return array("popular_tags" => $popular_tags, "assigned_tags" => $assigned_tags);
  }
}
