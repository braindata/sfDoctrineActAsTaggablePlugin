...
class PluginTagTable extends Doctrine_Table
{
    public static function getTagNamesGroupByModelId(array $ids, $modelName)
    {
      ...
      $modelTags = array();
      foreach($q->execute(null, Doctrine_Core::HYDRATE_ARRAY) as $tag) {
        $name = $tag['name'];
...
//        return array_keys($q->orderBy('t.name')->execute(array(), Doctrine_Core::HYDRATE_ARRAY));
    ...
    */
    public static function getObjectTaggedWithQuery($model, $tags = array(), Doctrine_Query $q = null, $options = array())
    {
        ...
        {
          $q->whereIn($rootAlias . '.id', $tagging);
        }
    ...
    */
    protected static function getTaggings($tags = array(), $options = array())
    {
        ...
        // by JO: if no ids are found with given tags
        if (count($tag_ids) == 0)
          return array();
